...
// @downloadURL  https://n8n.euro.ro/webhook/425a5c53-c882-4dc2-8d8a-287e56c69410
// @updateURL    https://n8n.euro.ro/webhook/425a5c53-c882-4dc2-8d8a-287e56c69410
// ==/UserScript==
...
(function () {
  ...
  const VOSS_CHAT = BASE + 'voss_chat.png';
  const VOSS_HOME = BASE + 'voss_project.png';
  const VOSS_BG = BASE + 'voss_background.png';
  ...
  const PROJECTS = [
    ...
    { id: 'template-builder', projectId: '019dc9fc-5001-741a-9648-4788558df268', label: 'Template Builder', extends: 'foundry' },
    {
      ...
      accentColor: '#8a6a3c',
      chatBackground: 'linear-gradient(160deg, #1a1408 0%, #141008 30%, #0e0c06 60%, #0a0804 100%)',
      card: { imageUrl: VOSS_CARD, titleColor: '#8a6a3c', letterSpacing: '0.5px', textTransform: null },
      chat: { backgroundImage: VOSS_BG, characterUrl: VOSS_CHAT, characterOpacity: 1.0, characterHeight: '72vh', characterBottom: '-90px', characterRight: '-180px' },
      homepage: { backgroundImage: VOSS_BG, characterUrl: VOSS_HOME, characterOpacity: 1.0, characterWidth: '500px', characterBottom: '-40px', characterRight: '-20px' },
    },
    {
    ...
    },
    {
      ...
      accentColor: '#4a8a8c',
      chatBackground: 'linear-gradient(160deg, #080a12 0%, #0a0e1a 30%, #080c14 60%, #06080e 100%)',
      card: { imageUrl: MERIDIAN_CARD, titleColor: '#4a8a8c', letterSpacing: '0.5px', textTransform: null },
    ...
    },
    {
      ...
      homepage: { backgroundImage: ADJUTANT_BG, characterUrl: ADJUTANT_HOME, characterOpacity: 0.85, characterWidth: '500px', characterBottom: '-40px', characterRight: '-20px' },
    },
    ...B_PROJECTS,
  ];
  ...
  function applyPersonaState(project, personaKey, persona) {
    ...
    if (isNew || changed) charEl.style.cssText = 'position:fixed;pointer-events:none;z-index:-1;user-select:none;'
    ...
    for (const p of PROJECTS) {
      ...
      for (const link of links) {
        ...
        for (const el of allEls) {
          ...
          if (hasText) {
            el.setAttribute('data-tm-card-title', '1');
        ...
        for (const el of allEls) {
          ...
          const t = (el.textContent||'').trim(); if (!t || el.hasAttribute('data-tm-card-title') || t.includes('Updated')) continue;
  ...
  function colorChatLinks() {
    ...
    if (window.location.pathname === '/recents' || window.location.pathname === '/cowork/recents') {
      ...
      for (const td of document.querySelectorAll('td')) {
        ...
        if (S.ACCOUNT === 'B') {
          const ui = titleText.indexOf('_');
          if (ui > 0) {
            const pk = titleText.substring(0, ui).toLowerCase(), pp = B_PERSONAS[pk];
            if (pp) {
              titleSpan.style.color = pp.accentColor;
              titleSpan.setAttribute(SIDEBAR_ATTR, pk);
              const bLabel = td.querySelector('span[class*="max-w-"]');
              if (bLabel) { bLabel.style.color = pp.accentColor; bLabel.style.opacity = '0.7'; }
              continue;
            }
          }
        }
  ...
  function refreshUtilBar() {
    ...
    if (inboxEl) {
      ...
      if (iData && iAttention > 0) {
        ...
        const iProjTotal = typeof iData?.projects_total === 'number' ? iData.projects_total : 0;
        const projNote = iProjTotal > 0 ? ', ' + iData.projects_total + ' project' + (iData.projects_total !== 1 ? 's' : '') : '';
        const fetchedAge = iData._fetchedAt ? '\nFetched: ' + formatAge(new Date(iData._fetchedAt)) : '';
        inboxEl.title = breakdown + projNote + (iData.total > iAttention ? ' of ' + iData.total + ' total' : '') + '\nUpdated: ' + age + (stale ? ' (stale)' : '') + fetchedAge;
      } else {
  ...
  function buildStylesheet(project, mode, cfg) {
    ...
    return `
      ...
      @keyframes thm-char-in { from{opacity:0;transform:translateY(12px)} to{opacity:var(--tm-char-opacity,1);transform:translateY(0)} }
      ...
      :root { --tm-accent: ${accent}; --tm-char-opacity: ${cfg.characterOpacity ?? 1}; }
...
${!isChat && project.account !== 'B' ? `      [${THEME_ATTR}] fieldset[data-tm-version="mismatch"]::before { content:'' !important;position:absolute;inset:-60px;border-radius:50%;background:radial-gradient(ellipse,#c9a84cf0 0%,#c9a84ca0 25%,#c9a84c60 50%,#c9a84c30 70%,transparent 90%);z-index:-1;pointer-events:none;animation:tm-version-pulse 2.5s ease-in-out infinite; }
  ...
  if (REDUCED_MOTION) {
    ...
    rm.textContent = '#' + BG_ID + ', #' + CHARACTER_ID + ' { animation: none !important; opacity: var(--tm-char-opacity,1) !important; } #' + CHARACTER_ID + ' img { animation: none !important; }';
